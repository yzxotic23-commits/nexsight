...
const cache = new Map()
const CACHE_DURATION = 30 * 1000 // 30 seconds in milliseconds (reduced for faster data updates)
 ...
 * @param {string} url - The URL to fetch
 * @param {object} options - Fetch options
 * @param {number} cacheDuration - Cache duration in milliseconds (default: 30 seconds)
 * @param {boolean} forceRefresh - If true, bypass cache and fetch fresh data
 */
export async function cachedFetch(url, options = {}, cacheDuration = CACHE_DURATION, forceRefresh = false) {
  ...
  if (!forceRefresh) {
    ...
    // Force refresh: clear cache for this key
    cache.delete(cacheKey)
  }
